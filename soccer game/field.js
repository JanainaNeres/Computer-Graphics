...
function FieldDraw(ctx){
    ...
    ctx.beginPath();
    ctx.arc( (this.size.x/2) - this.offset.x, (this.size.y/2) - this.offset.y, 60, 0 , 2*Math.PI);
    ...
    // Top D
    ctx.beginPath();  
    ctx.arc( (this.size.x/2) - this.offset.x,  200 - this.offset.y, 40, 0 , Math.PI);
    ctx.stroke();

    // Bottom D
    ctx.beginPath();
    ctx.arc( (this.size.x/2) - this.offset.x,  (this.pitchsize.y + 10) - 190 - this.offset.y, 40, Math.PI, 0 );
    ctx.stroke();
