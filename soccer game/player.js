...
function PlayerUpdate(field){
    ...
    if(this.loc.x > field.size.x - 25){
        this.loc.x = field.size.x -25;
    }

    if(this.loc.y < 30){
        this.loc.y = 30;
    }
    if(this.loc.y > field.size.y - 30){
        this.loc.y = field.size.y - 20;
    }
...
function PlayerDraw(ctx){
    ...
    ctx.fill();
    ctx.beginPath();
    ctx.ellipse(new_foot_x, new_foot_y, foot_size_x, foot_size_y, this.rotation + (Math.PI/2), 1, 2 * Math.PI);
    ctx.stroke();
    ...
    var head_size = parseInt(this.size.y * 0.85);
